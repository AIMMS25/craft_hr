...
def generate_abbreviation(leave_type):
    """
    Generate an abbreviation for a leave type based on its name.
    This is a basic implementation; you might need a more sophisticated method depending on your requirements.
    """
...
for leave in leave_types:
    ...
    abbr = generate_abbreviation(leave_type)
    status_map[leave_type] = abbr
...
for leave in leave_types:
	leave_type = leave.get('name')
	if not leave_type or leave_type in status_map_1:
		continue

	abbreviation = ''.join([word[0].upper() for word in leave_type.split()])
	status_map_1[leave_type] = abbreviation
...
def get_message() -> str:
	...
	count = 0
	for status, abbr in status_map_1.items():
		color = colors[count % color_count]
...
def get_attendance_map(filters: Filters) -> dict:
    ...
    for d in attendance_list:
        if d.status == "On Leave":
            leave_map.setdefault(d.employee, []).append((d.day_of_month, d.leave_type))
            continue
    ...
    for employee, leave_days in leave_map.items():
        ...
        for day, leave_type in leave_days:
            for shift in attendance_map[employee].keys():
                attendance_map[employee][shift][day] = leave_type
...
def get_attendance_status_for_detailed_view(
	...
	"""
	Returns a list of shift-wise attendance status for employee:
	[
	...
	for shift, status_dict in employee_attendance.items():
		...
		for day in range(1, total_days + 1):
			...
			if status is None and holidays:
				status = get_holiday_status(day, holidays)
			abbr = status_map.get(status, status)
			row[cstr(day)] = abbr
