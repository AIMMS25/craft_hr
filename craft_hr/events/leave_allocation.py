...
def validate(doc,method):
    ...
    if doc.custom_is_earned_leave and doc.custom_leave_distribution_template and doc.docstatus = 0:
        total_opening_leaves = get_leaves(doc.custom_date_of_joining,doc.from_date, doc.custom_leave_distribution_template) or 0
        used_leaves = total_opening_leaves - doc.custom_opening_leaves
        doc.db_set("custom_used_leaves", used_leaves)
        doc.db_set("custom_opening_used_leaves", used_leaves)
        doc.db_set("new_leaves_allocated", doc.custom_opening_leaves)
        doc.db_set("custom_available_leaves", doc.custom_opening_leaves)
