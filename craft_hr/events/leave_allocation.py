...
def validate(doc,method):
    ...
    if doc.custom_is_earned_leave and doc.custom_leave_distribution_template:
        ...
        doc.custom_used_leaves = flt(total_opening_leaves) - flt(doc.custom_opening_leaves)
        doc.custom_opening_used_leaves = flt(total_opening_leaves) - flt(doc.custom_opening_leaves)
        doc.new_leaves_allocated = flt(doc.custom_opening_leaves)
        doc.custom_available_leaves = flt(doc.custom_opening_leaves)
        doc.total_leaves_allocated=20
